...
ofr (let i = 0; i <numHexagons; i++){
    createHexagon();
}

window.addEventListener('resize'){
    const currentHexagons = contrainer. querySelectorAll('.hexagon'). length;

    const targeHexagons =Math.ceil((window.innerWidth * indow.innerHeight) / (50 * 50));
}